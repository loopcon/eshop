...
// Login as seller
$(document).on('submit', '.seller-login-form-submit-event', function (e) {
    e.preventDefault();
    var formData = new FormData(this);
    var form_id = $(this).attr("id");
    var error_box = $('#error_box', this);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result['error'] == true) {
                ...
                submit_btn.html(button_text);
                submit_btn.attr('disabled', false);
                $('.seller-login-form-submit-event')[0].reset();
                setTimeout(function () { window.location.href = base_url + 'my-account'; }, 600);
...
$(document).on('click', '.add_to_cart', function (e) {
    ...
    if(is_loggedin==0) {
        var is_guest = localStorage.getItem('is_guest');
        var guest_user_id = localStorage.getItem('guest_user_id');
        if(is_guest==null) {
            is_guest = 1; // 1 : Yes
            localStorage.setItem("is_guest", is_guest);
        }
        if(guest_user_id==null || guest_user_id==0) {
            guest_user_id = Math.floor(Date.now() / 1000);
...
$(document).ready(function () {
    ...
    if (cart) {
        display_cart(cart);
        // display_cart_product_list(cart);
    }
});
...
// function display_cart_product_list(cart) {
//     if (is_loggedin == 0) {
//         var cart_count = (cart.length) ? cart.length : "";
//         var html = '';
//         if (cart !== null && cart.length > 0) {
//             cart.forEach((item) => {
//                 html += '<tr class="cart-product-desc-list">'
//                     + '<td><div class="widget-image"><a href="'+base_url+'products/details/" target="_blank"><img src="'+item.image+'" alt="" /></a></div></td>'
//                     + '<td></td>'
//                     + '</tr>/;'
//             });
//         }
//         $('#cart_item_table tbody').html(html);
//     }
// }
...
// New js validation, and other events
...
$(document).on('submit', '#forgot_password_form', function (e) {
    e.preventDefault();
    // var send_otp_btn = $('#forgot_password_send_otp_btn').html();
    $('#forgot_password_submit_btn').html('Please Wait...').attr('disabled', true);
    // var phoneNumber = $('.selected-dial-code').html() + $('#forgot_password_number').val();
    var email = $('#forgot_password_email').val();
    var response = check_user_email(email);
    if (response.error == true) {
        $('#forgot_pass_error_box').html("You have not registered using this number.");
        $('#forgot_password_send_otp_btn').html(send_otp_btn).attr('disabled', false);
    } else {
        ...
        // firebase.auth().signInWithPhoneNumber(phoneNumber, appVerifier).then(function (confirmationResult) {
            // resetRecaptcha();
            $('#forgot_password_new_password_form').removeClass('d-none');
...
$(document).on('submit', '#forgot_password_new_password_form', function (e) {
    e.preventDefault();
    var reset_pass_btn_html = $('#reset_password_submit_btn').html();
    // var code = $('#forgot_password_otp').val();
    var formdata = new FormData(this);
    ...
    // confirmationResult.confirm(code).then(function (result) {
        formdata.append(csrfName, csrfHash);
        formdata.append('email', $('#forgot_password_email').val());
        $.ajax({
            type: 'POST',
            url: url,
            data: formdata,
...
$(document).on('change', '#add-address-form #country', function() {
    var formdata = new FormData();
    formdata.append(csrfName, csrfHash);
    var country_id = $(this).val();
    formdata.append('country_id', country_id);
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#add-address-form #state').html(html);
        }
    })
});

$(document).on('change', '#add-address-form #state', function() {
    var formdata = new FormData();
    formdata.append(csrfName, csrfHash);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#add-address-form #u-city').html(html);
        }
    })
});
...
/* $(document).on('change', '#add-address-form #u-city', function() {
    ...
    formdata.append(csrfName, csrfHash);
    var country_id = $('#add-address-form #country').val();
    var state_id = $('#add-address-form #state').val();
    var city_id = $(this).val();
    formdata.append('country_id', country_id);
    formdata.append('state_id', state_id);
    formdata.append('city_id', city_id);
...
$(document).on('submit', '#register-as-seller-form', function (e) {
    ...
    var formdata = new FormData(this);
    var url = $(this).attr('action');
    $('#register_seller_submit_btn').html('Please Wait...').attr('disabled', true);
    formdata.append(csrfName, csrfHash);
    $.ajax({
        type: 'POST',
        url: url,
...
function check_user_email(email) {
    ...
    $.ajax({
        type: 'POST',
        async: false,
        url: base_url + 'auth/check_user_email',
        data: { email: email, [csrfName]: csrfHash },
        dataType: 'json',
        success: function (result) {
            csrfName = result['csrfName'];
...
$(document).on('submit', '#store-details-form', function (e) {
    e.preventDefault();
    var store_detail_submit_btn = $('#save-store-detail-submit-btn').html();
    $('#save-store-detail-submit-btn').html('Please Wait...').attr('disabled', true);
    var formdata = new FormData(this);
    var url = $(this).attr('action');
    formdata.append(csrfName, csrfHash);
    $.ajax({
        ...
        success: function (result) {
            csrfName = result.csrfName;
            csrfHash = result.csrfHash;
            $('#save-store-detail-submit-btn').html(store_detail_submit_btn).attr('disabled', false);
            $("#save-store-detail-result").html(result.message).show();
            if(result.store_logo!='') {
                $('.lg-img').removeClass('d-none');
                $('.lg-img img').attr('src', base_url + result.store_logo);
            }
            if(result.store_logo!='') {
                $('.nid-img').removeClass('d-none');
                $('.nid-img img').attr('src', base_url + result.national_identity_card);
            }
            // if (result.error == false) {
            //     setTimeout(function () {
            //         window.location.reload();
...
$(document).on("change", "#store_logo", function(e) {
    var store_logo = document.getElementById("store_logo");
    var canvas = document.createElement("canvas");
    canvas.width = store_logo.width;
    canvas.height = store_logo.height;
    console.log(canvas);
...
$('.navbar-toggler').click(function() {
    $('#openCategoryMenu').slideToggle('slow');
});
$('.level-1 ul li i.updown-arrow-level-1').click(function() {
    $(this).closest('li').find('.level-2').toggle();
    if($(this).closest('li').find('.level-2').is(":visible")) {
        $(this).addClass('fa-angle-down').removeClass('fa-angle-up');
    } else {
        $(this).addClass('fa-angle-up').removeClass('fa-angle-down');
    }
});
$('.level-1 .level-2 ul li i.updown-arrow-level-2').click(function() {
    $(this).closest('li').find('.level-3').toggle();
    if($(this).closest('li').find('.level-3').is(":visible")) {
        $(this).addClass('fa-angle-down').removeClass('fa-angle-up');
    } else {
        $(this).addClass('fa-angle-up').removeClass('fa-angle-down');
    }
