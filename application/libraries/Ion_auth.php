        ...
        if (!$email_activation) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                if(in_array(4, $group_ids)) {
                    ...
                    $this->email->to($email);
                    $this->email->subject($site_title . ' - ' . $this->lang->line('email_activation_subject'));
                    $this->email->message($message);
                    ...
                    if ($this->email->send() === TRUE) {
                        ...
                        // email send to admin
                        $admin_message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_admin_new_seller', 'ion_auth'), $data, true);
                        $this->email->clear();
                        $this->email->from("$admin_email", $site_title);
                        $this->email->to($admin_email);
                        $this->email->subject($site_title . ' - ' . $this->lang->line('email_admin_new_seller_heading'));
                        $this->email->message($admin_message);
                        $this->email->send();

                        return $id;
                    }
                } elseif(in_array(2, $group_ids)) {
                    $message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_welcome', 'ion_auth'), $data, true);
                    ...
                    $this->email->from($admin_email, $site_title);
                    $this->email->to($email);
                    $this->email->subject($site_title . ' - ' . $this->lang->line('email_welcome_subject'));
                    $this->email->message($message);
                    if ($this->email->send() === TRUE) {
                        $this->ion_auth_model->trigger_events(['post_account_creation', 'post_account_creation_successful', 'activation_email_successful']);
                        $this->set_message('activation_email_successful');
                        ...
                        // email send to admin
                        $admin_message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_admin_new_user', 'ion_auth'), $data, true);
                        $this->email->clear();
                        $this->email->from($admin_email, $site_title);
                        $this->email->to($admin_email);
                        $this->email->subject($site_title . ' - '. $this->lang->line('email_admin_new_user_subject'));
                        $this->email->message($admin_message);
                        $this->email->send();
