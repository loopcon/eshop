...
<ul class="nav nav-pills nav-justified flex-column bg-white rounded shadow p-3 mb-0 my-account-tab" id="pills-tab" role="tablist">
    ...
    <?php if($login_type=='vendor') { ?>
        <li class="nav-item mt-2">
            <a class="nav-link rounded <?= ($current_url == base_url('my-account/store-details')) ? 'active' : '' ?>" id="store-details" href="<?=base_url('my-account/store-details') ?>">
                <div class="text-left py-1 px-3 sidebar-text">
                    <h6 class="mb-0">
                        <i class="fas fa-store fa-lg left-aside"></i> <?= !empty($this->lang->line('store_details')) ? $this->lang->line('store_details') : 'Store detail' ?>
                    </h6>
                </div>
            </a>
        </li>
