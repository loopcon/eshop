...
class Home extends CI_Controller
{
    ...
    public function faq()
    {
        ...
        $this->data['faq'] = $this->faq_model->get_faqs(null, null, "id", "ASC");
        $this->load->view('front-end/' . THEME . '/template', $this->data);
    }
    ...
    public function login_seller()
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            $user_data = fetch_details('users', ['email' => $identity]);
            $seller_data = fetch_details('seller_data', ['user_id' => $user_data[0]['id']]);

            if (isset($seller_data[0]['status']) && !empty($seller_data) && $seller_data[0]['status'] == 7) {
                $response['error'] = true;
            ...
            if (!empty($res)) {
                ...
                if ($this->ion_auth->seller_login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                    //if the login is successful
                    if (!$this->input->is_ajax_request()) {
                        redirect('admin/home', 'refresh');
                    }
    ...
    public function reset_password()
    {
        ...
        */
        ...
        $identity_column = $this->config->item('identity', 'ion_auth');
        $res = fetch_details('users', ['email' => $_POST['email']]);
        if (!empty($res)) {
