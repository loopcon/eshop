...
class Auth extends CI_Controller
{
    ...
    public function register_seller()
    {
        ...
        $this->form_validation->set_rules('email', 'Mail', 'trim|required|xss_clean|valid_email|is_unique[users.email]', array('is_unique' => ' The email is already registered. Please use other email or login'));
        $this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|xss_clean|min_length[5]|numeric|is_unique[users.mobile]', array('is_unique' => ' The mobile number is already registered. Please use other mobile number'));
        ...
        if (!$this->form_validation->run()) {
            $this->response['error'] = true;
            $this->response['message'] = str_replace("\n", "<br>", strip_tags(validation_errors()));
            $this->response['data'] = array();
        } else {
            $identity_column = $this->config->item('identity', 'ion_auth');
    ...
    public function check_user_email()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if(!isset($_POST['email']) || $_POST['email']=='') {
                $this->response['error'] = true;
                $this->response['message'] = 'Please enter email!';
                $this->response['data'] = array();
                print_r(json_encode($this->response));
                return;
            } else {
                $data = $this->db->select('id')->where(['email' => $_POST['email']])->get('users u')->result_array();
                if(!empty($data)) {
                    $this->response['error'] = false;
                    ...
                    $this->response['error'] = true;
                    $this->response['message'] = 'You have not registered using this email.';
                    $this->response['data'] = array();
                    print_r(json_encode($this->response));
                    return;
                }
